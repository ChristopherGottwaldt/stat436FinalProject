...
births_url <- 'https://uwmadison.box.com/shared/static/0q0l4ed7apmpxkpuv7pfszqv4w4p6ugw.csv'
births_data <- read_csv(births_url) %>%
  mutate(across(-county, ~ifelse(. == "<5", "0", as.character(.)))) %>%
  mutate(across(-county, as.numeric))


counties <- sf::st_read("https://uwmadison.box.com/shared/static/irl834idoboilvcj9rhq8gujpfcdydsy.geojson", quiet = TRUE)
stats <- read_csv("https://uwmadison.box.com/shared/static/oln9vtfskaj7hp0l0q2o94m1vzlos77k.csv")
